...
constant const int kMaxSelectedClusters = 64;  // Max number of clusters that can be selected
...
typedef struct
{
    ...
    /*
     The first N splats are represented as as 2N primitives and 4N vertex indices. The remainder are represented
    ...
    int selectedClusterID;             // 4 bytes  (offset 148) -1 means show all clusters (single selection)
    ...
    uint selectedClusterCount;         // 4 bytes  (offset 168) number of selected clusters
